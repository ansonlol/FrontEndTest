...
contract AcornTest {
    ...
    function check_accounts(string memory account_name) public view returns(uint256,uint256,uint256,uint256,uint256[] memory,string[] memory){
        Souls memory temp = souls[account_name];
        return (temp.ACorn_tokens, temp.ACorn_earned,temp.ACorn_used,temp.Total_hours, temp.skillIndices,temp.skillName);
    }
    ...
    function check_skills(string memory account_name, string memory skill_name)public view returns(string memory, string[] memory, uint256){
        uint256 skillIndex = account_name_and_skill_to_index(account_name, skill_name);
        return (skills[skillIndex].level, skills[skillIndex].performance, skills[skillIndex].hour_spent);
    }
    ...
    function add_new_souls(string memory account_name) public {
        Souls memory soul = Souls(0, 0, 0, 0, new uint256[](0), new string[](0));
        souls[account_name] = (soul);
    }

    // each souls can join a new skill
    function join_skill(string memory account_name,string memory skill_name) public {
        Skills memory skill = Skills(skill_name, "Exploration", new string[](0), 0);
    ...
    function finish_skills_lesson(string memory skill_name,  string memory account_name, string memory performance, uint256 time) public {
        ...
        uint256 skill_index = account_name_and_skill_to_index(account_name, skill_name);
    ...
    function finish_skills_level(string memory account_name, string memory skill_name, string memory finish_level) public {
        uint256 tokens_earned_finish_level;
        string memory next_level;
        (tokens_earned_finish_level, next_level) = level_to_tokens(finish_level);
        //update profile
    ...
    event buy_history(string message);
    function buy_time_to_play( string memory account_name, uint256 time_minute) public{
        ...
        emit buy_history(string(abi.encodePacked("Buy time for ", time_minute.toString(), " minutes. Tokens Used: ", tokens_paid_to_time.toString())));
    ...
    function performance_to_token(string memory performance) public pure returns(uint256){
    ...
    function level_to_tokens(string memory finish_level) public pure returns(uint256, string memory){
        uint256 tokens_earn;
        string memory next_level;
    ...
    function time_play_to_tokens(uint256 time_minutes) public pure returns(uint256){
        uint tokens_pay;
        tokens_pay = time_minutes*2;
        return tokens_pay;
    }
    function account_name_and_skill_to_index(string memory account_name, string memory skill_name) public view returns(uint256){
